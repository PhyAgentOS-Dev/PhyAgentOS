...
> **当前实现范围（必须明确）**：RoboTwin/PAOS 当前只实现顺序双臂模式：一次只驱动一只选中机械臂，另一只机械臂保持 `hold`（或未来先执行显式 `park`），并作为静态碰撞投影参与规划。当前没有实现两只机械臂同时行动、统一时间轴、同步 swept-volume 检查、共同停止或原子双臂执行。两个独立 Curobo planner 分别规划成功，不能被解释为同步联合规划。
